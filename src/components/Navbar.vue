      ...
      <nav class="w-full flex justify-between">
        ...
        <div class="flex justify-self-start">
          <RouterLink :to="{ name: 'home' }">
            <img src="../assets/logo.png" alt="homeLogoApp" width="65" height="36" class="ml-9 flex justify-start"
        ...
        <div class="flex justify-evenly gap-60">
          <div @mouseenter="dropdownHandler(true)">
            ...
            <div @mouseover="dropdownHandler(true)" @mouseout="dropdownHandler(true)"
              ...
              }" v-for="(types, index) of typesOfItemsMan" :key="index" :id="index" @click="
  $emit(
    'filterByType',
    filterWithTypes(types, `men's clothing`)
              ...
              <RouterLink :to="{ name: 'type' }" @click="$emit('filterByType', filterWithSex(`men's clothing`))"
                class="transition duration-150 ease-in hover:text-[#FF8C00]">View All</RouterLink>
            </div>
          </div>
          ...
          <div @mouseenter="dropdownHandler(false)">
            ...
            <div @mouseover="dropdownHandler(false)" @mouseout="dropdownHandler(false)"
              class="w-full grid grid-cols-4 gap-4 absolute bg-[#9263B1] drop-shadow-2xl -left-0 top-14 -right-48 px-10 mt-2 pt-2 text-center"
              v-show="wDropdown">
              <RouterLink :to="{ name: 'type' }" @click="
                $emit(
                  'filterByType',
                  filterWithTypes(types, `women's clothing`)
                )
              " v-for="(types, index) of typesOfItemsWoman" :key="index" :id="index"
                class="transition duration-150 ease-in hover:text-[#FF8C00]">
                {{ types }}
              </RouterLink>
              <RouterLink :to="{ name: 'type' }" @click="
        ...
        <div
          ...
          <div class="flex items-center justify-center px-3 py-2 transition ease-in duration-150 hover:scale-110">
            <!-- search Icon -->
            <svg class="cursor-pointer max-sm:fill-white" @click="showSearchIcon = !showSearchIcon"
              v-show="!showSearchIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            ...
            <!-- close Icon -->
            <svg class="cursor-pointer" @click="clearSearch" v-show="showSearchIcon" xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="flex justify-self-end gap-6">
          <!-- cart button -->
          <RouterLink :to="{ name: 'cart' }" class="mr-10">
    ...
    <div v-show="dropdown" class="absolute w-full z-50">
      <div class="w-full drop-shadow-lg">
        <div v-show="index <= 4" v-for="(p, index) in searchFilter" :key="p.id"
